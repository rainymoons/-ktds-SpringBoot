...
//	private String baseDirectory;
	...
	@Override
	public BoardListVO getAllBoard(SearchBoardVO searchBoardVO) { // SearchBoardVO searchBoardVO 엑셀다운로드시 이건 null
		// 게시글 목록 화면에 데이터를 전송해주기 위해서 
		// 게시글의 건수와 게시글의 목록을 조회해 반환시킨다.
		
		// 1. 게시글의 건수를 조회 -> count가 0이면 동작할 필요가 없다. 조회시 성능저하.
		...
		if (boardCount == 0) {
			...
			return boardListVO;
		} // 위처럼 count가 0이면 아래 로직은 조회하지 않는다.
		
		// 2. 게시글의 목록을 조회 (엑셀다운로드 + 페이지네이션)
		// 엑셀 다운로드를 위한 게시글 조회
