...
// 		- ...
@EnableWebMvc
@EnableWebSocket // WebSocket을 활성화 시킨다.
public class WebConfig implements WebMvcConfigurer // WebMVC를 사용하기 위한 설정 인터페이스
	...
	/**
	 * WebSocket 구현
	 * WebSocket의 EndPoint(URL)를 설정.
	 * 해당 EndPoint(URL)의 통신을 담당할 클래스가 필요.
	 */
	@Override
	public void registerWebSocketHandlers(WebSocketHandlerRegistry registry) {
		// parameter : controller와 url
		registry.addHandler(this.textWebSocketHandler, "/ws")
				// CORS 설정. 모든 도메인(URL)에서 /ws로 접근할 수 있도록 설정.
				// 보안 정책 때문에, 네이버에서 카카오의 api접근이 불가능하다. 만약 *대신 http://www.kako.com을 추가 하면 naver에서 접근이 가능해진다.
				// * 로 설정하면 모든 도메인에서 접근이 가능하도록 설정해준다는 것.
				.setAllowedOrigins("http://localhost:8080")
				.withSockJS(); // /ws URL에 접근할 수 있는 JS라이브러리를 sock.js로 제한한다.
	}
}
