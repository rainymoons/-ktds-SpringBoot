...
public class SecurityConfig {
	...
	SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
		...
		/**
		 * 다른 도메인에서 9090번 포트로 요청을 하는 것에 대해 허용을 할 것인지 말 것인지를 정한다.
		 * 여기에 적어주는 요청만 허용을 해주겠다.
		 */
		http.cors( cors -> {
			...
			CorsConfigurationSource source = request -> {
				...
				// 허용을 할 도메인의 목록.
				corsConfiguration.setAllowedOrigins(List.of("http://localhost:8080")); // origin = 요청자의 도메인. 8080에 대한 요청을 허용해주겠다.
				// 허용을 할 메소드의 목록.
				// 외부에서 우리 Application으로 요청을 보낼 수 있다.
				corsConfiguration.setAllowedMethods(List.of("POST", "GET", "PUT", "DELETE", "OPTION")); // 댓글을 달기 위해 보냈던 요청은 POST.(GET, PUT, DELETE는 block)
